...
Ordinary PRs run selected checks on one Linux/Node 24 runner. Full verification runs on `main`, release tags, manual dispatch and high-impact PRs, using the minimum Node version, Node 24/26 and Linux/macOS/Windows. Duplicate branch-push runs are disabled. Require the final `CI` status in branch protection. A local pass does not establish that the remote matrix passed. See [testing](docs/testing.md) and [release policy](docs/release-policy.md).

A pull request that changes public behavior includes a changeset (`npm run changeset`). Releases are versioned by `.github/workflows/release.yml` through Changesets: merges to `main` accumulate changesets into a `chore: version packages` PR, and merging that PR runs the full `check:full` gate and then stages the release to npm via Trusted Publishing (OIDC) — no maintainer runs `npm publish` by hand, and no token is stored in this repository. Staging is not publishing: a maintainer still approves the staged version with 2FA (`npm stage approve <stage-id>`, or the Staged Packages tab on npmjs.com) before it becomes installable. See [release policy](docs/release-policy.md) for the staging/approval flow, how to retry a skipped or failed stage, and the current `GITHUB_TOKEN`-PR/CI workaround (close and reopen the version-packages PR to trigger its `CI` check).
