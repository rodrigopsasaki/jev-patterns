...
**GitHub tags and releases are no longer created automatically.** `changesets/action` v2 defaults to creating both after every publish step, regardless of what that step prints. `release.yml` pins `create-github-releases: false` and `push-git-tags: false` explicitly: a GitHub release or tag before a maintainer has actually approved the stage would announce a version that isn't installable yet. Tag and release creation, if wanted, is a manual step after approving the stage.
