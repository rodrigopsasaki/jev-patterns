...
export function dominant(
  options: { readonly floor?: number; readonly gap?: number } = {},
): Predicate {
  const { floor = defaultThresholds.dominant.floor, gap = defaultThresholds.dominant.gap } =
    options;
  requireProbability(floor, 'floor');
  requireProbability(gap, 'gap');
  return (data) =>
    data.uniqueMaximum !== null &&
    atLeast(data.maximumProbability, floor) &&
...
export function paired(
  options: { readonly floor?: number; readonly secondFloor?: number; readonly gap?: number } = {},
): Predicate {
  const {
    floor = defaultThresholds.paired.floor,
    secondFloor = defaultThresholds.paired.secondFloor,
    gap = defaultThresholds.paired.gap,
  } = options;
  requireProbability(floor, 'floor');
  ...
  return (data) =>
    ...
    data.second !== null &&
    atLeast(data.maximumProbability, floor) &&
    atLeast(data.secondProbability, secondFloor) &&
    atLeast(data.gap, gap);
}

/** gap bounds the first-to-second difference; combinedFloor bounds their combined probability. */
export function split(
  options: { readonly gap?: number; readonly combinedFloor?: number } = {},
): Predicate {
  const {
    gap = defaultThresholds.split.gap,
    combinedFloor = defaultThresholds.split.combinedFloor,
  } = options;
  requireProbability(gap, 'gap');
  requireProbability(combinedFloor, 'combinedFloor');
  return (data) =>
    data.second !== null &&
    atLeast(gap, data.gap) &&
...
export function clustered(
  ...
  const {
    minimumCount = defaultThresholds.clustered.minimumCount,
    combinedFloor = defaultThresholds.clustered.combinedFloor,
    ratio = defaultThresholds.clustered.ratio,
  } = options;
  ...
  return (data) => {
    const items = data.sorted.filter(
      (item) => item.probability > 0 && atLeast(item.probability / data.maximumProbability, ratio),
    );
    return (
      items.length >= minimumCount &&
      atLeast(
        items.reduce((total, item) => total + item.probability, 0),
        combinedFloor,
      )
    );
  };
...
export function flat(
  options: { readonly minimumRatio?: number; readonly minimumCount?: number } = {},
): Predicate {
  const {
    minimumRatio = defaultThresholds.flat.minimumRatio,
    minimumCount = defaultThresholds.flat.minimumCount,
  } = options;
...
function requireCount(value: number): number {
  if (!Number.isSafeInteger(value) || value < 2)
    throw new TypeError('minimumCount must be an integer >= 2');
  return value;
}
