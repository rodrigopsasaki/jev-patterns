...
test('uniform distributions have k effective options with no unique winner', () => {
...
test('zero padding and insertion order preserve descriptors', () => {
  ...
  for (const key of ['shape', 'contenders', 'metrics', 'massSet', 'leaders', 'input']) {
...
test('small floating point drift is reported; malformed mass is rejected', () => {
  ...
  for (const value of [0, -1, NaN, Infinity, 1.1]) {
    assert.throws(() => analyze({ a: 1 }, { contenderRatio: value }), TypeError);
