...
import { allOf, anyOf, not } from '../src/predicates.ts';
...
const examples = {
  ...
  mixed: { a: 0.6, b: 0.1, c: 0.1, d: 0.1, e: 0.1 },
};
  ...
  for (const shape of shapes) {
    for (const invalid of invalidValues) {
      ...
      assert.throws(() => distribution.match(handlers), {
        name: 'TypeError',
        message: `match() requires a handler for ${JSON.stringify(shape)}`,
      });
...
test('maxima retain every tied maximum in deterministic order and share outcome references', () => {
  ...
  assert.deepEqual(
    tied.maxima.map((outcome) => outcome.option),
    ['a', 'm', 'z'],
  );
