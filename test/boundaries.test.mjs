...
import {
  analyze, massSet, dominant, paired, split, clustered, flat,
  maximumProbabilityAtLeast, gapAtLeast,
} from '../src/index.ts';

const fromWeights = weights => {
  const total = weights.reduce((a, b) => a + b, 0);
  return analyze(Object.fromEntries(weights.map((weight, index) => [`option-${index}`, weight / total])));
};
...
test('minimum and maximum predicate thresholds are inclusive, with both adjacent sides exercised', () => {
  const boundaries = [
    { name: 'dominant default floor', at: .8, factory: () => dominant(), data: p => [p, 1 - p] },
    { name: 'dominant custom floor', at: .7, factory: () => dominant({ floor: .7 }), data: p => [p, 1 - p] },
    { name: 'dominant default gap', at: .2, factory: () => dominant({ floor: 0 }), data: g => [(1 + g) / 2, (1 - g) / 2] },
    { name: 'dominant custom gap', at: .3, factory: () => dominant({ floor: 0, gap: .3 }), data: g => [(1 + g) / 2, (1 - g) / 2] },
    { name: 'paired default floor', at: .5, factory: () => paired(), data: p => [p, .3, 1 - p - .3] },
    { name: 'paired custom floor', at: .55, factory: () => paired({ floor: .55 }), data: p => [p, .3, 1 - p - .3] },
    { name: 'paired default second floor', at: .2, factory: () => paired(), data: p => [.6, p, (.4 - p) / 2, (.4 - p) / 2] },
    { name: 'paired custom second floor', at: .25, factory: () => paired({ secondFloor: .25 }), data: p => [.6, p, (.4 - p) / 2, (.4 - p) / 2] },
    { name: 'paired default gap', at: .15, factory: () => paired(), data: g => [.55, .55 - g, g - .1] },
    { name: 'paired custom gap', at: .2, factory: () => paired({ gap: .2 }), data: g => [.55, .55 - g, g - .1] },
    { name: 'split default maximum gap', at: .1, maximum: true, factory: () => split(), data: g => [(.9 + g) / 2, (.9 - g) / 2, .1] },
    { name: 'split custom maximum gap', at: .2, maximum: true, factory: () => split({ gap: .2 }), data: g => [(.9 + g) / 2, (.9 - g) / 2, .1] },
    { name: 'split default combined floor', at: .75, factory: () => split(), data: m => [(m + .04) / 2, (m - .04) / 2, (1 - m) / 3, (1 - m) / 3, (1 - m) / 3] },
    { name: 'split custom combined floor', at: .85, factory: () => split({ combinedFloor: .85 }), data: m => [(m + .04) / 2, (m - .04) / 2, (1 - m) / 3, (1 - m) / 3, (1 - m) / 3] },
    { name: 'clustered default ratio', at: .5, factory: () => clustered(), data: r => [1, .85, r, .05] },
    { name: 'clustered custom ratio', at: .65, factory: () => clustered({ ratio: .65 }), data: r => [1, .85, r, .05] },
    { name: 'clustered default combined floor', at: .75, factory: () => clustered(), data: m => [.4 * m, .33 * m, .27 * m, ...Array(4).fill((1 - m) / 4)] },
    { name: 'clustered custom combined floor', at: .65, factory: () => clustered({ combinedFloor: .65 }), data: m => [.4 * m, .33 * m, .27 * m, ...Array(4).fill((1 - m) / 4)] },
    { name: 'flat default minimum ratio', at: .75, factory: () => flat(), data: r => [1, .9, r] },
    { name: 'flat custom minimum ratio', at: .85, factory: () => flat({ minimumRatio: .85 }), data: r => [1, .9, r] },
    { name: 'maximum probability helper', at: .65, factory: () => maximumProbabilityAtLeast(.65), data: p => [p, (1 - p) / 2, (1 - p) / 2] },
    { name: 'minimum gap helper', at: .2, factory: () => gapAtLeast(.2), data: g => [(1 + g) / 2, (1 - g) / 2] },
  ];
...
test('default and custom minimum counts count positive support and include the exact minimum', () => {
  for (const [name, factory] of [['clustered', clustered], ['flat', flat]]) {
    for (const minimum of [2, 3, 5]) {
      for (const count of [minimum - 1, minimum, minimum + 1]) {
        const data = fromWeights([...Array(count).fill(1), 0, 0]);
        assert.equal(data.is(factory({ minimumCount: minimum })), count >= minimum,
          `${name}, minimum=${minimum}, positive count=${count}`);
        if (minimum === 3) assert.equal(data.is(factory()), count >= minimum, `${name} default count`);
      }
    }
  }
});
...
test('prominence and requested mass honor custom values on both sides of a boundary', () => {
  const input = { a: .5, b: .3, c: .2, zero: 0 };
  for (const offset of [-STEP, 0, STEP]) {
    const ratio = .6 + offset;
    const target = .8 + offset;
    const result = analyze(input, { prominenceRatio: ratio, targetMass: target });
    assert.deepEqual(result.prominent.items.map(item => item.option), offset <= 0 ? ['a', 'b'] : ['a']);
    assert.deepEqual(result.massSet.options.map(item => item.option), offset <= 0 ? ['a', 'b'] : ['a', 'b', 'c']);
...
test('relative threshold tolerance scales with the threshold, including tiny custom ratios', () => {
  ...
  for (const threshold of [.6, 1e-8, 1e-200]) {
    const values = [threshold * (1 - 2 * tolerance), threshold * (1 - tolerance / 2), threshold,
      threshold * (1 + tolerance / 2), threshold * (1 + 2 * tolerance)];
    for (const [index, ratio] of values.entries()) {
      const result = fromWeights([1, ratio]);
      const input = Object.fromEntries(result.sorted.map(item => [item.option, item.probability]));
      const prominent = analyze(input, { prominenceRatio: threshold });
      assert.equal(prominent.prominent.count, index === 0 ? 1 : 2,
        `relative threshold=${threshold}, actual ratio=${ratio}`);
      assert.equal(result.is(clustered({ minimumCount: 2, combinedFloor: 0, ratio: threshold })), index !== 0);
    }
  }
  const result = analyze({ a: .6, b: .4 });
  assert.equal(result.is(maximumProbabilityAtLeast(.6 * (1 + tolerance / 2))), true);
  assert.equal(result.is(maximumProbabilityAtLeast(.6 * (1 + 2 * tolerance))), false);
...
test('absolute tie tolerance determines maxima and expands mass boundaries without chaining ties', () => {
  ...
  for (const nominalGap of [0, tolerance / 2, tolerance, 2 * tolerance]) {
    const input = { a: .5 + nominalGap / 2, b: .5 - nominalGap / 2 };
    ...
    assert.equal(result.maxima.length, isTie ? 2 : 1, `nominal gap=${nominalGap}, represented gap=${actualGap}`);
    assert.equal(result.uniqueMaximum === null, isTie);
    assert.equal(massSet(input, .4).count, isTie ? 2 : 1);
  }
  const nearChain = analyze({ a: 1/3 + .75 * tolerance, b: 1/3, c: 1/3 - .75 * tolerance });
  assert.deepEqual(nearChain.maxima.map(item => item.option), ['a', 'b']);
  assert.deepEqual(massSet(Object.fromEntries(nearChain.sorted.map(item => [item.option, item.probability])), .2)
    .options.map(item => item.option), ['a', 'b']);
});
...
test('mass tolerance is inclusive for partial requests and never removes positive support at target one', () => {
  ...
  for (const tail of [Number.EPSILON / 2, Number.MIN_VALUE]) {
    ...
    assert.deepEqual(selected.options.map(item => item.option), ['a', 'b']);
  }
});
...
test('accepted sum drift is normalized and its represented boundary is enforced on both sides of one', () => {
  ...
  for (const sign of [-1, 1]) {
    for (const scale of [.5, 1, 2]) {
      const input = { a: .6, b: .4 + sign * scale * tolerance };
      ...
      if (Math.abs(total - 1) > tolerance) {
        ...
        assert.throws(() => massSet(input, .8), TypeError, label);
      } else {
        const result = analyze(input);
  ...
  assert.deepEqual(analyze({ a: .75, b: .25 }).input, { total: 1, normalized: false });
});
...
test('numeric parameter validation rejects malformed values across every factory and analysis option', () => {
  ...
  for (const [name, factory] of probabilityOptions) {
    for (const value of [null, NaN, Infinity, -Infinity, -.01, 1.01, '0.5', true, false, {}, [], 1n]) {
      assert.throws(() => factory(value), TypeError, `${name}, invalid ${String(value)}`);
    }
  }
  for (const factory of [clustered, flat]) {
    for (const value of [null, -1, 0, 1, 1.5, NaN, Infinity, Number.MAX_SAFE_INTEGER + 1, '3', true, {}, [], 3n]) {
      assert.throws(() => factory({ minimumCount: value }), TypeError, `minimumCount=${String(value)}`);
    }
  }
...
test('valid endpoint parameters and explicit undefined defaults remain usable', () => {
  ...
  assert.deepEqual(analyze({ a: .6, b: .4 }, { prominenceRatio: 1, targetMass: Number.MIN_VALUE })
    .prominent.items.map(item => item.option), ['a']);
  const input = { a: .6, b: .3, c: .1 };
  ...
  for (const [factory, keys] of [
    [dominant, ['floor', 'gap']], [paired, ['floor', 'secondFloor', 'gap']],
    [split, ['gap', 'combinedFloor']], [clustered, ['minimumCount', 'combinedFloor', 'ratio']],
    [flat, ['minimumRatio', 'minimumCount']],
  ]) assert.equal(defaults.is(factory(Object.fromEntries(keys.map(key => [key, undefined])))), defaults.is(factory()));
});
