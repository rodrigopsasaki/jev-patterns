...
function numericView(result) {
  return {
    ...
    secondProbability: result.secondProbability,
    gap: result.gap,
    input: result.input,
...
test('distribution observations are invariant under insertion order and option renaming', () => {
  ...
  checkCases((input) => {
    const options = { prominenceRatio: 0.65, targetMass: 0.91 };
    ...
    const renamed = analyze(
      Object.fromEntries(entries.map(([, p], index) => [renamedKeys[index], p])),
      options,
    );
    assert.deepEqual(numericView(renamed), numericView(original));
    const probabilitiesByName = (items) =>
      Object.fromEntries(
        [...items]
          .sort((a, b) => (a.option < b.option ? -1 : a.option > b.option ? 1 : 0))
          .map((item) => [item.option, item.probability]),
      );
    assert.deepEqual(
      probabilitiesByName(
        renamed.sorted.map((item) => ({
          ...item,
          option: reverseNames.get(item.option),
        })),
      ),
    ...
    for (const items of ['maxima', 'sorted']) {
      assert.deepEqual(
        names(renamed[items].map((item) => ({ ...item, option: reverseNames.get(item.option) }))),
...
test('exact zero padding preserves positive-support observations and probability groups', () => {
  checkCases((input) => {
    const options = { prominenceRatio: 0.3, targetMass: 0.97 };
    const original = analyze(input, options);
    ...
    assert.deepEqual(
      padded.sorted.filter((item) => item.probability > 0),
...
test('metrics satisfy probability, entropy and effective-number bounds with conserved group mass', () => {
  checkCases((input) => {
    const result = analyze(input, { prominenceRatio: 0.4, targetMass: 0.87 });
    const positive = result.sorted.filter((item) => item.probability > 0);
    const support = positive.length;
    const slack = 2e-11;
    const bounded = [
      result.maximumProbability,
      result.secondProbability,
      result.gap,
      result.metrics.firstTwoProbability,
      result.prominent.probability,
      result.prominent.remainingProbability,
      result.massSet.mass,
      result.massSet.excludedMass,
    ];
    ...
    for (let index = 0; index < result.sorted.length; index++) {
      ...
      close(
        item.probability,
        input[item.option] / sum(Object.values(input)),
    ...
    const tied = result.sorted.filter(
      (item) =>
        result.maximumProbability - item.probability <=
    ...
    close(
      result.metrics.firstTwoProbability,
      sum(result.sorted.slice(0, 2).map((item) => item.probability)),
      'first-two mass',
    );
    const {
      entropyNats,
      effectiveOptions: { shannon, simpson },
    } = result.metrics;
    assert.ok(
      Number.isFinite(entropyNats) &&
        entropyNats >= -slack &&
        entropyNats <= Math.log(support) + slack,
    );
    ...
    for (const item of result.sorted) {
      ...
      if (prominentNames.has(item.option)) {
        ...
        assert.ok(
          ratio >= 0.4 * (1 - result.profile.numericTolerances.thresholdRelative) - Number.EPSILON,
        );
      } else
        assert.ok(
          item.probability === 0 || ratio < 0.4,
          'an outcome at the cutoff must be prominent',
        );
    }

    for (const group of [
      {
        items: result.prominent.items,
        count: result.prominent.count,
        retained: result.prominent.probability,
        remaining: result.prominent.remainingProbability,
      },
      {
        items: result.massSet.options,
        count: result.massSet.count,
        retained: result.massSet.mass,
        remaining: result.massSet.excludedMass,
      },
    ]) {
      ...
      close(
        ...
        sum(
          result.sorted
            .filter((item) => !selected.has(item.option))
  ...
  checkCases((input) => {
    ...
    for (const multiplicity of [2, 3]) {
      const split = Object.fromEntries(
        Object.entries(input).flatMap(([key, probability]) =>
          Array.from({ length: multiplicity }, (_, index) => [
            `${key}/part-${index}`,
      ...
      close(
        result.metrics.entropyNats,
        original.metrics.entropyNats + Math.log(multiplicity),
        'split entropy',
      );
      for (const kind of ['shannon', 'simpson']) {
        close(
          result.metrics.effectiveOptions[kind],
          original.metrics.effectiveOptions[kind] * multiplicity,
          `split ${kind}`,
          2e-9,
        );
      }
...
test('mass sets are nested prefixes, meet tolerated coverage and retain every exact boundary tie', () => {
  checkCases((input) => {
    ...
    for (const target of [Number.MIN_VALUE, 0.01, 0.2, 0.5, 0.8, 0.95, 0.999, 1]) {
      const selected = massSet(input, target);
      const selectedNames = new Set(selected.options.map((item) => item.option));
      assert.ok(
        [...previous].every((name) => selectedNames.has(name)),
        `nested set at ${target}`,
      );
      ...
      assert.ok(
        selected.mass + result.profile.numericTolerances.massBoundaryAbsolute + 2e-14 >= target,
      );
      ...
      if (target === 1)
        assert.deepEqual(
          names(selected.options),
          names(result.sorted.filter((item) => item.probability > 0)),
        );
...
test('distinct-probability mass sets are minimal within the declared numerical tolerance', () => {
  ...
  for (let sample = 0; sample < 50; sample++) {
    ...
    const prefixMass = [];
    for (const item of result.sorted) {
      prefixMass.push((prefixMass.at(-1) ?? 0) + item.probability);
    }
    const targets = [
      0.1,
      0.5,
      0.8,
      0.99,
      ...prefixMass.slice(0, -1).flatMap((mass) => [mass - 1e-7, mass, mass + 1e-7]),
    ].filter((target) => target > 0 && target < 1);
    for (const target of targets) {
      const selected = massSet(input, target);
      const previousMass = sum(selected.options.slice(0, -1).map((item) => item.probability));
      assert.ok(
        previousMass + result.profile.numericTolerances.massBoundaryAbsolute < target,
...
test('analysis and mass selection leave frozen inputs and options unchanged', () => {
  checkCases((input) => {
    ...
    const options = Object.freeze({ prominenceRatio: 0.7, targetMass: 0.85 });
    Object.freeze(input);
    const result = analyze(input, options);
    assert.deepEqual(massSet(input, options.targetMass), result.massSet);
    assert.deepEqual(input, before);
    assert.deepEqual(options, { prominenceRatio: 0.7, targetMass: 0.85 });
  });
});
